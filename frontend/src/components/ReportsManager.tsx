      ...
      <div style={{ ...card, padding: '14px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            {TABS.map(tab => {
              ...
              return (
                <button key={tab.id}
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 14px', borderRadius: 10, border: `1.5px solid ${active ? tab.color : 'var(--border-color)'}`, background: active ? tab.bg : 'var(--border-color)', color: active ? tab.color : 'var(--text-muted)', fontSize: 12, fontWeight: active ? 800 : 600, cursor: 'pointer', transition: 'all 0.15s' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            {activeTab === 'stock' && (
              <>
                <Filter style={{ width: 14, height: 14, color: 'var(--text-light)' }} />
            ...
            {(activeTab === 'transfers' || activeTab === 'profit') && (
              <>
                <Calendar style={{ width: 14, height: 14, color: 'var(--text-light)' }} />
                ...
                <button onClick={loadReportData}
                  style={{ padding: '9px 14px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: 'var(--bg-card)', fontWeight: 700, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 4px 12px rgba(124,58,237,0.3)', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {stockSummary && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
              {[
                { label: 'Product Lines', value: `${stockRecords.length}`, sub: 'unique SKUs', icon: <Layers style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#7c3aed, #4f46e5', glow: 'rgba(124,58,237,0.3)' },
                { label: 'Total Cost Value', value: `LKR ${(stockSummary.totalCostVal / 1000).toFixed(1)}K`, sub: 'at purchase price', icon: <DollarSign style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#f59e0b, #d97706', glow: 'rgba(245,158,11,0.3)' },
                { label: 'Wholesale Value', value: `LKR ${(stockSummary.totalWholesaleVal / 1000).toFixed(1)}K`, sub: 'at wholesale price', icon: <ShoppingCart style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#06b6d4, #0891b2', glow: 'rgba(6,182,212,0.3)' },
                { label: 'Retail Value', value: `LKR ${(stockSummary.totalRetailVal / 1000).toFixed(1)}K`, sub: 'at retail price', icon: <TrendingUp style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#059669, #047857', glow: 'rgba(5,150,105,0.3)' },
              ].map(s => (
          ...
          <div style={{ ...card, padding: '16px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, paddingBottom: 12, borderBottom: '1px solid #f1f5f9' }}>
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: 'var(--text-main)' }}>Inventory Balances Report</h3>
                <p style={{ margin: 0, fontSize: 11, color: 'var(--text-light)' }}>{stockRecords.length} records{selectedLocId ? ` · filtered` : ' · all locations'}</p>
              </div>
            </div>
            {loading ? (
              <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-light)', fontSize: 13 }}>Loading report…</div>
            ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720 }}>
                  ...
                  <tbody>
                    {stockRecords.length > 0 ? stockRecords.map((r, idx) => (
                      <tr key={r.stockId} style={{ background: idx % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-input)' }}>
                        <td style={{ ...td }}>
                        ...
                        <td style={{ ...td, fontWeight: 700, color: 'var(--text-main)' }}>{r.itemName}</td>
                        ...
                        <td style={{ ...td, textAlign: 'right', fontFamily: 'monospace', color: 'var(--text-muted)', fontSize: 11 }}>LKR {r.cost.toFixed(2)}</td>
                        <td style={{ ...td, textAlign: 'right', fontFamily: 'monospace', color: 'var(--text-muted)', fontSize: 11 }}>LKR {r.wholesalePrice.toFixed(2)}</td>
                        <td style={{ ...td, textAlign: 'right', fontFamily: 'monospace', color: 'var(--text-muted)', fontSize: 11 }}>LKR {r.retailPrice.toFixed(2)}</td>
                      ...
                      <tr><td colSpan={9} style={{ ...td, textAlign: 'center', padding: '40px', color: 'var(--text-light)' }}>No stock records found.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div style={{ ...card, overflow: 'hidden' }}>
              <div style={{ background: 'linear-gradient(135deg,#92400e,#b45309)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Search style={{ width: 15, height: 15, color: 'var(--bg-card)' }} />
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: 'var(--bg-card)' }}>Warranty Tracker</h3>
              ...
              <form onSubmit={handleWarrantySearch} style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <button type="submit" disabled={loading || !serialQuery.trim()}
                  style={{ padding: '10px', borderRadius: 10, border: 'none', background: (!serialQuery.trim() || loading) ? 'var(--border-color)' : 'linear-gradient(135deg,#f59e0b,#d97706)', color: (!serialQuery.trim() || loading) ? 'var(--text-light)' : 'var(--bg-card)', fontWeight: 700, fontSize: 13, cursor: (!serialQuery.trim() || loading) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ ...card, overflow: 'hidden', minHeight: 300 }}>
              <div style={{ background: 'linear-gradient(135deg,#1e1b4b,#312e81,#4c1d95)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Award style={{ width: 15, height: 15, color: 'var(--bg-card)' }} />
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: 'var(--bg-card)' }}>Lifecycle Record</h3>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div style={{ padding: '14px 16px', borderRadius: 12, border: `2px solid ${warrantyResult.warrantyAvailability.includes('Active') ? '#86efac' : '#fca5a5'}`, background: warrantyResult.warrantyAvailability.includes('Active') ? '#f0fdf4' : '#fef2f2', display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 38, height: 38, borderRadius: 10, background: warrantyResult.warrantyAvailability.includes('Active') ? '#10b981' : '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: `0 4px 10px ${warrantyResult.warrantyAvailability.includes('Active') ? 'rgba(16,185,129,0.4)' : 'rgba(239,68,68,0.4)'}` }}>
                          <Award style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: 9, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase' }}>Warranty Status</p>
                          ...
                          {warrantyResult.warrantyExpiry && (
                            <p style={{ margin: '2px 0 0', fontSize: 10, color: 'var(--text-light)' }}>Expires: {new Date(warrantyResult.warrantyExpiry).toLocaleDateString()}</p>
                          )}
                        </div>
                      </div>
                    </div>
                      ...
                      <div style={{ padding: '14px 16px', background: 'rgba(124,58,237,0.05)', borderRadius: 12, border: '1.5px solid rgba(124,58,237,0.15)' }}>
                        ...
                        {[
                          ...
                          <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(124,58,237,0.08)' }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-main)' }}>{r.value}</span>
                          </div>
                        ))}
                      </div>
                        ...
                        <div style={{ padding: '14px 16px', background: 'rgba(239,68,68,0.04)', borderRadius: 12, border: '1.5px solid rgba(239,68,68,0.2)', flex: 1 }}>
                          ...
                          {[
                            ...
                            <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(239,68,68,0.08)' }}>
                              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{r.label}</span>
                              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-main)', fontFamily: r.label === 'Invoice #' ? 'monospace' : 'inherit' }}>{r.value}</span>
                            </div>
                          ))}
                        </div>
                      ) : (
      ...
      {activeTab === 'transfers' && (
        <div style={{ ...card, overflow: 'hidden' }}>
          <div style={{ background: 'linear-gradient(135deg,#0e7490,#0891b2)', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: 'var(--bg-card)' }}>Stock Transfers Log</h3>
              <p style={{ margin: 0, fontSize: 10, color: 'rgba(255,255,255,0.55)' }}>{transferRecords.length} transfers · {startDate} to {endDate}</p>
            </div>
          </div>
          <div style={{ padding: '16px 20px', overflowX: 'auto' }}>
            {loading ? (
              <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-light)', fontSize: 13 }}>Loading transfers…</div>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
                ...
                <tbody>
                  {transferRecords.length > 0 ? transferRecords.map((rec, idx) => (
                    <tr key={rec.id} style={{ background: idx % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-input)' }}>
                      <td style={{ ...td, fontFamily: 'monospace', fontSize: 11, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{new Date(rec.date).toLocaleString()}</td>
                      <td style={{ ...td }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                          ...
                          <ArrowRightLeft style={{ width: 12, height: 12, color: 'var(--text-light)', flexShrink: 0 }} />
                      ...
                      <td style={{ ...td, textAlign: 'center' }}>
                        {rec.transferItems.map((ti, i) => (
                          <p key={i} style={{ margin: '1px 0', fontSize: 13, fontWeight: 800, color: 'var(--text-main)', fontFamily: 'monospace' }}>{ti.quantity}</p>
                        ))}
                      </td>
                      <td style={{ ...td, textAlign: 'center' }}>
                        {rec.transferItems.map((ti, i) => (
                      ...
                      <td style={{ ...td, fontSize: 11, color: 'var(--text-muted)', fontStyle: 'italic', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={rec.reason}>{rec.reason}</td>
                    </tr>
                  )) : (
                    <tr><td colSpan={7} style={{ ...td, textAlign: 'center', padding: '40px', color: 'var(--text-light)' }}>No transfers in this date range.</td></tr>
                  )}
                </tbody>
              </table>
            )}
      ...
      {activeTab === 'profit' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {profitSummary && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
              {[
                { label: 'Total Revenue', value: `LKR ${(profitSummary.totalRevenue / 1000).toFixed(1)}K`, full: `LKR ${profitSummary.totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}`, icon: <DollarSign style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#7c3aed, #4f46e5', glow: 'rgba(124,58,237,0.3)' },
                { label: 'Cost of Goods (COGS)', value: `LKR ${(profitSummary.totalCogs / 1000).toFixed(1)}K`, full: `LKR ${profitSummary.totalCogs.toLocaleString('en-US', { minimumFractionDigits: 2 })}`, icon: <ShoppingCart style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#f59e0b, #d97706', glow: 'rgba(245,158,11,0.3)' },
                { label: 'Net Profit', value: `LKR ${(profitSummary.totalProfit / 1000).toFixed(1)}K`, full: `LKR ${profitSummary.totalProfit.toLocaleString('en-US', { minimumFractionDigits: 2 })}`, icon: <TrendingUp style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#059669, #047857', glow: 'rgba(5,150,105,0.3)' },
                { label: 'Profit Margin', value: `${profitMargin}%`, full: `${profitMargin}% of revenue`, icon: <BarChart2 style={{ width: 18, height: 18, color: 'var(--bg-card)' }} />, grad: '#06b6d4, #0891b2', glow: 'rgba(6,182,212,0.3)' },
              ].map(s => (
          ...
          {profitSummary && profitSummary.totalRevenue > 0 && (
            <div style={{ ...card, padding: '14px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: 'var(--text-muted)' }}>Revenue Breakdown</p>
                <p style={{ margin: 0, fontSize: 11, color: 'var(--text-light)' }}>{profitMargin}% profit margin</p>
              </div>
              <div style={{ height: 12, background: 'var(--border-color)', borderRadius: 6, overflow: 'hidden', display: 'flex' }}>
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>COGS ({(profitSummary.totalCogs / profitSummary.totalRevenue * 100).toFixed(1)}%)</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 10, height: 10, borderRadius: 2, background: '#059669' }} />
                  <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Net Profit ({profitMargin}%)</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ ...card, overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(135deg,#064e3b,#047857)', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: 'var(--bg-card)' }}>Profitability Ledger</h3>
                <p style={{ margin: 0, fontSize: 10, color: 'rgba(255,255,255,0.55)' }}>{profitRecords.length} invoices · {startDate} to {endDate}</p>
              </div>
            </div>
            <div style={{ padding: '16px 20px', overflowX: 'auto' }}>
              {loading ? (
                <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-light)', fontSize: 13 }}>Loading profit data…</div>
              ) : (
                  ...
                  <tbody>
                    {profitRecords.length > 0 ? profitRecords.map((rec, idx) => (
                      <tr key={rec.id} style={{ background: idx % 2 === 0 ? 'var(--bg-card)' : 'var(--bg-input)' }}>
                        ...
                        <td style={{ ...td, fontFamily: 'monospace', fontSize: 11, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{new Date(rec.date).toLocaleDateString()}</td>
                        ...
                        <td style={{ ...td, textAlign: 'right', fontFamily: 'monospace', color: 'var(--text-main)', fontWeight: 700 }}>LKR {rec.revenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                      ...
                      <tr><td colSpan={8} style={{ ...td, textAlign: 'center', padding: '40px', color: 'var(--text-light)' }}>No sales data in this date range.</td></tr>
                    )}
                  </tbody>
                </table>
              )}
